...
do
    ...
    do
        if [[ "$file" == "insert_trace-400e7-100-add-no-dup-MONOTONIC_INCREASE.bin" ]]; then
            outdir="./../results_Motivation_w_hints/search/MI"
        else
            outdir="./../results_Motivation_w_hints/search/SU"
        fi

        mkdir -p "$outdir"
        total_ops=$((2 * ops))
        ./uvm-opt.out -ops="$total_ops" -add="$ops" -fil=1 -rem=0 -rns=1 -tra="$file" -gbs=100000000 > "${outdir}/${ops}.log"
    done
done
