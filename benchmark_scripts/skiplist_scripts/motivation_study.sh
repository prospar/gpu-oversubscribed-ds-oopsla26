...
trace_type=( "MonotonicIncrease" "SparseUnique")
...
do
    ...
    do
        ...
        # $BIN_DIR/$bench.out -ops=$oper -add=$oper -rem=0 -tra="insert_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-SPARSE_UNIQUE.bin" -trf="search_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-no-absent-SPARSE_UNIQUE.bin" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 > $skiplist_results_dir/$expr_dir/$bench/$size/SparseUnique/output.log 2>&1
        $BIN_DIR/$bench.out -ops=$oper -add=$oper -rem=0 -tra="insert_trace-25e7-400e7-100-add-0-rem-0-find-no-dup-MONOTONIC_INCREASE.bin" -trf="insert_trace-25e7-400e7-100-add-0-rem-0-find-no-dup-MONOTONIC_INCREASE.bin" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 > $skiplist_results_dir/$expr_dir/$bench/$size/MonotonicIncrease/output.log 2>&1
        # insert for sparse trace takes too long to complete, beyond 60% oversubscription single iteration takes around 4 days
        # $BIN_DIR/$bench.out -ops=$oper -add=$oper -rem=0 -tra="insert_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-DENSE_UNIQUE.bin" -trf="search_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-no-absent-SPARSE_UNIQUE.bin" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 > $skiplist_results_dir/$expr_dir/$bench/$size/SparseUnique/output-search.log 2>&1
        # $BIN_DIR/$bench.out -ops=$oper -add=$oper -rem=0 -tra="insert_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-SPARSE_UNIQUE.bin" -trf="" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 > $skiplist_results_dir/$expr_dir/$bench/$size/SparseUnique/output.log 2>&1
