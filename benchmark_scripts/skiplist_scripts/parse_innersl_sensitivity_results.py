...
def extract_output_number(filepath):
    """
    output-rng120.log -> 120
    """
    filename = os.path.basename(filepath)
    m = re.search(r"output-rng(\d+)", filename)
    if m:
        return m.group(1)
    return "Unknown"
...
def parse_file(filepath, results):
    ...
    try:
        ...
        rng_size = extract_output_number(filepath)
        record = {
        ...
        if trace_type not in results:
            ...
            results[trace_type] = {
                "Insert": {
                    "Trace Type": f"{trace_type} Insert"
                },
                "Search": {
                    "Trace Type": f"{trace_type} Search"
                }
            }

        results[trace_type]["Insert"][f'2^{rng_size}'] = record["Insert Time"]
        results[trace_type]["Search"][f'2^{rng_size}'] = record["Search Time"]
...
def walk_directory(root_dir):
    ...
    # print(results)
    rows = []
    for trace in sorted(results):
        rows.append(results[trace]["Insert"])
        rows.append(results[trace]["Search"])
    
    # return list(results.values())
    return rows


def write_csv(results, outfile="parsed_results_table10.csv"):
