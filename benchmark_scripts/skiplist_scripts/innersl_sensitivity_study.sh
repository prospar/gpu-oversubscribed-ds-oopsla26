...
expr_dir="innersl_sensitivity_study"
...
# the result in paper are generated with 100% ovesubcription
num_ops=( "5000000000" )

bench_list=( "sl-ovs-baseline" )

rng_list=( "12" "15" "18" "21" "24" "27" "30" )
...
do
    ...
    do
        ...
        for range in "${rng_list[@]}"
        do
            $BIN_DIR/$bench.out -ops=$size -add=$oper -rem=0 -tra="insert_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-DENSE_UNIQUE.bin" -trf="search_trace-25e7-800e7-50-add-0-rem-50-find-no-dup-no-absent-DENSE_UNIQUE.bin" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 -rng=$range > $skiplist_results_dir/$expr_dir/$bench/$size/DenseUnique/output-rng$range.log 2>&1
            $BIN_DIR/$bench.out -ops=$size -add=$oper -rem=0 -tra="insert_trace-25e7-400e7-100-add-0-rem-0-find-no-dup-MONOTONIC_INCREASE.bin" -trf="insert_trace-25e7-400e7-100-add-0-rem-0-find-no-dup-MONOTONIC_INCREASE.bin" -trr="" -blk=15625000 -siz=512 -rns=1 -gbs=250000000 -rng=$range > $skiplist_results_dir/$expr_dir/$bench/$size/MonotonicIncrease/output-rng$range.log 2>&1
        done
    done
