...
public:
  ...
  Chunk *memory_pool;
  Chunk *fixed_memory_pool;
  uint32_t pool_size;
  uint32_t uvm_allocated; // counter for getNewChunk
  uint32_t fixed_allocated;
...
__host__ GFSL::GFSL(int max_nodes, int ratio, bool host_side) {
  ...
  pool_size = max_nodes; // 1 for head node of level 0
  int fixed_chunks = (max_nodes + TEAM_SIZE) / ratio;
  // head node of each level + one head
  cudaCheckErrorMacro(
      cudaMalloc((void **)&fixed_memory_pool, sizeof(Chunk) * fixed_chunks),
      "Allocation of fixed pool failed");
  fixed_allocated = TEAM_SIZE;

  pool_size = max_nodes + 1;
  uvm_allocated = 1; //first nodes for each level
  cudaCheckErrorMacro(
      cudaMallocManaged(&memory_pool, sizeof(Chunk) * (pool_size)),
      "Allocation of data layer memory pool failed"); //extra node for head
...
__host__ bool GFSL::initializeGFSL(int max_nodes, float ratio, bool host_side) {
  ...
  // one head node, and starting node for each level
  // pool_offset = max_nodes; // help to identify memory pool type
  int fixed_chunks = (max_nodes * ratio) + TEAM_SIZE + 1;
  cudaCheckErrorMacro(
      cudaMalloc(&fixed_memory_pool, sizeof(Chunk) * fixed_chunks),
      "Mem Allocation for index layer failed");
  fixed_allocated = TEAM_SIZE; // head node and first node of each index level

  pool_size = (max_nodes * (1.05 - ratio)) + 1;
  // cudaCheckErrorMacro(
  //     cudaMallocManaged(&memory_pool, sizeof(Chunk) * pool_size),
  //     "Mem allocation for data layer failed");
  printf("Index layer pool: %d\n", fixed_chunks);
  printf("Data layer pool: %d\n", pool_size);

  // head data track the number of chunks in each level
  uint64_t *head_data = fixed_memory_pool->all_data_ptr();
  head = fixed_memory_pool++; // next Chunk is the start of the mem pool
  ...
  Chunk device_side_chunk;
  uint64_t *device_chunk_data = device_side_chunk.all_data_ptr();
  // level 1 node downpointer will be for uvm pool, explicilty change it
  device_chunk_data[0] = (KV{.kv = {.key = NEG_INF, .value = CHK_INVALID}}).raw;
  for (int i = 1; i < TID_LOCK; i++) {
    device_chunk_data[i] =
        (KV{.kv = {.key = POS_INF, .value = CHK_INVALID}}).raw;
  ...
  for (int i = 1; i < TEAM_SIZE; i++) {
    ...
    device_chunk_data[0] =
        (KV{.kv = {.key = NEG_INF, .value = static_cast<unsigned int>(i - 1)}})
            .raw;
    cudaCheckErrorMacro(
        cudaMemcpy(fixed_memory_pool[i].all_data_ptr(), device_chunk_data,
                   sizeof(uint64_t) * 32, cudaMemcpyHostToDevice),
        "Mem copy failure during initialization of index layer chunk");
  }
...
__device__ ChunkIdx GFSL::getNewChunk(int level) {
  ...
  int tid = getMyTid();
  if (tid == TID_LOCK) {
    // FIXED_INDEX: update to use different memory pool based on level
    if (level == 0) {
      idx = atomicInc(static_cast<unsigned int *>(&(this->uvm_allocated)),
                      POS_INF);
    } else {
      idx = atomicInc(static_cast<unsigned int *>(&(this->fixed_allocated)),
                      POS_INF);
    }
  }
...
// FIXED_INDEX: update the method to fetch chunk from memory pool based on level
__device__ Chunk *GFSL::getChunkFromIdx(ChunkIdx idx, int level) {
  //     if(idx == CHK_INVALID){
  //         printf("Something is very wrong\n");
  //     }
  if (level > 0) // use level instead of idx
    return &(fixed_memory_pool[idx]);
  else
    return &(memory_pool[idx]);
}
...
restart:
  ...
  while (height > 0) {
    ...
    if (stepTid == TID_NEXT) {
      ...
      height--;
      pPrev = CHK_INVALID;
      // K temp = __shfl_sync(teamMask, currKv.kv.key, stepTid, TEAM_SIZE); // debug
      pCurr = getPtrFromTid(stepTid, currKv);
